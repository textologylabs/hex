...
> **"cannot rebuild the X baseline — the template at … is now Y".**

The app has no `.hex/pristine/` (it was generated by an older Hex), so the merge
base has to be re-rendered from the template it came from — and that template has
since moved on to another version. Merging against a re-render of Y would silently
drop Y's own changes, so Hex refuses.

Put a copy of the template at version X somewhere (`git worktree add
../template-X vX`), point `root.source.path` in `.hex/lockfile.yaml` at it, and
re-run. The upgrade stores a baseline as it goes, so you only do this once. Apps
generated by Hex 1.0+ carry their own baseline and never hit this — see
[the pristine baseline](./upgrade.md#the-pristine-baseline--hexpristine).
